...
def trans_vector_matrix(dic,max_diff_time): #matrix to compare masks
...
def lineage_matrix(dic,maskslist,mat_vec,mat_ang,max_diff_time,threshold):
    ...
    for i in tqdm.trange(len(diclist)-1):
        ...
        while abs(timer-base_time)<=max_diff_time:
            ...
            links_p,links_c =ot_cost_matrix(mask_c,mask_p,area_c,area_p,threshold)
...
def ot_cost_matrix(mask_c, mask_p, area_c, area_p, threshold):
    ...
    #1 if mask, 0 for background 
    mask_bin_p = (mask_p > 0).astype(np.uint8)  
    mask_bin_c = (mask_c > 0).astype(np.uint8) 

    coords_p = np.argwhere(mask_p >0)  
    coords_c = np.argwhere(mask_c >0) 

    a = np.ones(len(coords_p))    / len(coords_p)
    b = np.ones(len(coords_c))    / len(coords_c)
    print("shape ", coords_c.shape)
    print("size ", coords_p.shape)
    ...
    # ot.plot.plot2D_samples_mat(coords_p, coords_c, T, c=[0.6, 0.6, 0.6])
    # plt.plot(coords_p[:, 1], coords_p[:, 0], "ob", label="parent")  
    # plt.plot(coords_c[:, 1], coords_c[:, 0], "or", label="child")
